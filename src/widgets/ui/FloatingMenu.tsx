...
const FloatingMenu = () => {
     ...
     */
    const menuItems = [
        { key: 'top', type: 'a', to: '#', icon: <ArrowUp size={20} />, text: '맨 위로', action: scrollToTop },
        { key: 'notice', type: 'link', to: '/notices', icon: <Megaphone size={20} />, text: '공지사항', action: closeMenu },
        { key: 'help', type: 'link', to: '/help', icon: <LifeBuoy size={20} />, text: '고객센터', action: closeMenu },
        { key: 'profile', type: 'link', to: '/profile', icon: <User size={20} />, text: '프로필', action: closeMenu },
        { key: 'dm', type: 'link', to: '/dm', icon: <MessagesSquare size={20} />, text: 'DM', action: closeMenu },
    ];
    // --- /수정된 메뉴 아이템 ---
            ...
            <div className="flex flex-col items-center mb-4 space-y-2.5">
              ...
              `}
                        ...
                        >
                            {/* 툴팁 */}
                            <span
                                className="absolute right-full top-1/2 z-10 mr-3 w-20 -translate-y-1/2 rounded-md bg-gray-800 py-1.5 text-center text-sm text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100 invisible group-hover:visible"
                            >
                {item.text}
                                {/* 툴팁 화살표 (Tailwind border로 구현) */}
                                <div
                                    className="absolute left-full top-1/2 h-0 w-0 -translate-y-1/2 border-y-[5px] border-l-[5px] border-y-transparent border-l-gray-800 border-r-transparent"
                                />
              </span>
