...
const HotAuctionItem: FC<Props> = ({ item }) => {
    ...
    }, [item.auction.user.id, navigate]); // id, navigate 의존성 추가

    return (
        <div className={'w-full cursor-pointer'} onClick={onClickItem}> {/* cursor-pointer 추가 */}

            {/* 1. 이미지 (relative 컨테이너로 변경) */}
            <div className='w-full aspect-square overflow-hidden relative'> {/* relative 추가 */}
